/**
 * Wealth · Alerts — the sweep's knobs, and whether it is actually running.
 *
 * Alerts only. Anything that describes *what you own* — wallets, off-chain assets, how amounts are
 * displayed — lives on Wealth · Settings, because "which wallets do I hold" is not a question
 * anyone goes looking for under a bell icon.
 *
...
export function WealthAlertsPage() {
  ...
  //
  // Done **during render** rather than in an effect. An effect paints the empty form first and
  // overwrites it on the next pass, which is the cascade `react-hooks/set-state-in-effect` exists
  // to catch. Identity comparison is enough: react-query hands back a new object only when the
  // data actually changed, so a refetch returning the same config does not clobber an edit in
  // progress.
  const [seededFrom, setSeededFrom] = useState<AlertStatus | null>(null)
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 sm:grid-cols-3">
          ...
          />
          <StatCard label="Wallets watched" value={String(status.wallets)} hint="the sweep's EVM wallets" />
        </CardContent>
      </Card>
    </div>
  )
