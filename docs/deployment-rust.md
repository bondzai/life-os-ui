...
The stack is defined in `docker-compose.rust.yml`. The older TypeScript stack in
`docker-compose.yml` is untouched and still works, so you can run this one alongside it and
switch over when you're happy.
...
```bash
docker compose -f docker-compose.rust.yml run --rm migrate \
...
```bash
docker compose -f docker-compose.rust.yml ps          # api should be "healthy"
...
```bash
docker compose -f docker-compose.rust.yml exec ollama ollama pull llama3.2:1b
```
...
Policy has to allow that origin. Out of the box `nginx.rust.conf` allows `localhost:11434` only.
...
then `docker compose -f docker-compose.rust.yml restart ui`. The file is bind-mounted, so no
...
## 3. Running next to the old stack

Both stacks can run at once, which is the point of the separate file — you cut over only when
the new one has proven itself.

- Containers, network and volumes are namespaced under the project name `lyra-rust`, so nothing
  is shared or overwritten. The two stacks have **separate databases**.
- The Rust API's port 3001 is not published, so it does not clash with the old API.
- The **UI port does clash**. Both want 8080. Either stop the old stack (`docker compose down`)
  or set `UI_PORT=8081` in `.env` and reach the new UI on `http://<mini-pc>:8081`.

When you are ready to cut over for good: `docker compose down` for the old stack, remove
`UI_PORT` from `.env`, and `docker compose -f docker-compose.rust.yml up -d` to take 8080.
...
3. Uncomment the `litestream` service in `docker-compose.rust.yml`.
4. `docker compose -f docker-compose.rust.yml up -d litestream`

Verify it is actually replicating — a backup you never checked is not a backup:

```bash
docker compose -f docker-compose.rust.yml logs litestream | tail
docker compose -f docker-compose.rust.yml exec litestream litestream snapshots /data/lyra.db
```
...
```bash
docker compose -f docker-compose.rust.yml stop api
...
docker compose -f docker-compose.rust.yml start api
...
docker compose -f docker-compose.rust.yml run --rm litestream \
...
```bash
docker compose -f docker-compose.rust.yml ps       # who is up, who is healthy
docker compose -f docker-compose.rust.yml logs -f api
```
...
| `api` restarts forever, health never goes green | DB unreadable — health runs a real query, so this is storage, not the process | `docker compose -f docker-compose.rust.yml logs api`; check the volume: `docker run --rm -v lyra-rust_lyra-data:/data alpine ls -l /data` |
...
| UI loads, calls to `/api/...` return 502 | nginx started before the API was healthy, or the API is down | `docker compose -f docker-compose.rust.yml restart ui` after the API is healthy |
...
| AI shows offline | Browser cannot reach Ollama, or the CSP blocks it | Open devtools → Console; a CSP violation means you need to add the origin to `nginx.rust.conf` (section 2.4). Otherwise check `curl http://<mini-pc>:11434/api/tags` |
...
`docker compose -f docker-compose.rust.yml up -d api`.
